...
INCLUDE_NAMED_STOCKS = True        # auch ohne WKN genannte Aktien (z. B. Vossloh) versuchen
WATCHLIST = ["Vossloh"]            # diese Namen IMMER aufnehmen (zuverlaessig, manuell pflegbar)
...
# Haeufige deutsche Woerter / Kontextbegriffe, die KEINE Aktien sind
STOPWORDS = {
    "die", "der", "das", "und", "oder", "aber", "heute", "ausserdem", "außerdem",
    "auch", "wir", "uns", "ueber", "über", "fuer", "für", "von", "mit", "bei",
    "aus", "den", "dem", "ein", "eine", "einen", "im", "in", "am", "zum", "zur",
    "aktie", "aktien", "boerse", "börse", "podcast", "prozent", "milliarden",
    "millionen", "dollar", "euro", "waffenruhe", "trump", "analyst", "analysten",
    "experten", "markt", "maerkte", "märkte", "index", "sektor", "strategie",
    "idee", "ideen", "folge", "thema", "themen", "news", "anleger", "kurs", "kurse",
    "iran", "usa", "china", "europa", "deutschland", "welt", "zukunft",
}

YH_HOSTS = ("query1.finance.yahoo.com", "query2.finance.yahoo.com")

def _yget(path, params, tries=3):
    """GET gegen Yahoo mit Wiederholversuchen ueber beide Hosts (gegen Throttling)."""
...
# 2) WKNs parsen
# ---------------------------------------------------------------------------
WKN_PATTERN = re.compile(r"([^()]{3,90}?)\s*\(WKN:\s*([A-Z0-9]{6})\)")

def clean_name(raw):
    """Aus dem Text vor "(WKN:" den eigentlichen Wertpapiernamen herausziehen
    (letztes Element einer Aufzaehlung, fuehrende Konjunktionen entfernt)."""
    s = re.split(r"[;:,]", raw)[-1]
    s = re.split(r"\s+(?:und|sowie|&|/)\s+", s)[-1]
    s = re.sub(r"^(?:und|sowie)\s+", "", s.strip(), flags=re.I)
    return s.strip(" .\t-")
...
def extract_wkns(description_html):
    ...
    for m in WKN_PATTERN.finditer(text):
        ...
        if wkn not in seen:
            seen.add(wkn)
            found.append({"wkn": wkn, "name": clean_name(m.group(1))})
    return found
...
# ---------------------------------------------------------------------------
def yahoo_lookup(query):
    """Bestes Yahoo-Quote-Objekt zu einer Suche; bevorzugt deutsche Notierungen."""
    if not query:
        return None
    data = _yget("/v1/finance/search",
                 {"q": query, "quotesCount": 8, "newsCount": 0})
    quotes = (data or {}).get("quotes", [])
    if not quotes:
        return None
    de = [q for q in quotes
          if str(q.get("symbol", "")).endswith((".DE", ".F", ".SG", ".MU", ".BE"))
          or q.get("exchange") in ("GER", "XETRA", "STU", "FRA", "MUN", "BER")]
    return (de or quotes)[0]


def yahoo_symbol(query):
    q = yahoo_lookup(query)
    return q.get("symbol") if q else None


def yahoo_chart(symbol):
    data = _yget(f"/v8/finance/chart/{symbol}",
...
def check_tradable(wkn, name):
    ...
    if not api_key or not cse_id:
        return None
    if wkn and wkn not in ("-", "\u2014"):
        query = f"Ist WKN {wkn} {name} bei Trade Republic handelbar?"
    else:
        query = f"Ist die Aktie {name} bei Trade Republic handelbar?"
    try:
...
# ---------------------------------------------------------------------------
# Genannte Aktien ohne WKN (z. B. Vossloh) - Watchlist + Best-Effort-Auswertung
# ---------------------------------------------------------------------------
_SPLIT = re.compile(r"[,.;:!?/()\"\u2013]|\bund\b|\bsowie\b|\bmit\b|\bvon\b|\bbei\b")

def _candidates_from_text(text):
    text = re.sub(r"\([^)]*WKN:[^)]*\)", " ", text)      # WKN-getaggte Teile raus
    out = []
    for part in _SPLIT.split(text):
        p = part.strip(" -\t")
        words = p.split()
        if not (2 <= len(p) <= 40 and 1 <= len(words) <= 4):
            continue
        if not p[0].isupper():
            continue
...
def collect_named_stocks(episodes, since, taken_symbols):
    """Watchlist-Namen (immer) + aus den Beschreibungen erkannte Aktien.
    Nur Treffer vom Typ EQUITY, mit Namensabgleich gegen Fehltreffer."""
    queue = [("Watchlist", None, w) for w in WATCHLIST]
    if INCLUDE_NAMED_STOCKS:
        for ep in episodes_in_range(episodes, since):
            d = episode_date(ep)
            desc = html.unescape(re.sub(r"<[^>]+>", " ",
                   ep.get("description") or ep.get("shortDescription") or ""))
            for cand in _candidates_from_text(desc):
                queue.append((ep.get("trackName", ""), d, cand))

    items, seen = [], set(taken_symbols)
    for ep_title, d, cand in queue:
        q = yahoo_lookup(cand)
        if not q or q.get("quoteType") != "EQUITY":
            continue
        sym = q.get("symbol")
        if not sym or sym in seen:
            continue
        nm = (q.get("shortname") or q.get("longname") or "").strip()
        first = next((w for w in cand.lower().split()
                      if len(w) >= 3 and w not in STOPWORDS), "")
        if first and nm and first not in nm.lower():
            continue                                  # Treffer passt nicht zum Namen
        seen.add(sym)
        items.append({"wkn": "\u2014", "name": nm or cand, "_symbol": sym,
                      "ep_title": ep_title, "ep_date": d})
        time.sleep(0.8)
    return items
...
<style>
  ...
  <p class="sub">{n_eps} Folge(n) ab {since} ausgewertet - {n} Werte insgesamt</p>
...
def main():
    ...
    rows, used_symbols = [], []
    for it in items:
        m = fetch_metrics(it["wkn"], it["name"])
        if m.get("symbol"):
            used_symbols.append(m["symbol"])
    ...
    # Zusaetzlich genannte Aktien ohne WKN (Watchlist + Auto-Erkennung)
    named = collect_named_stocks(episodes, SINCE_DATE, used_symbols)
    if named:
        print(f"Genannte Aktien ohne WKN: {len(named)}")
    for it in named:
        m = fetch_metrics(it["wkn"], it["name"], symbol=it.get("_symbol"))
        tr = check_tradable(it["wkn"], it["name"])
        print(f"  -> Aktie  {it['name'][:28]:28} 1J={m['y1']} 5J={m['y5']} Div={m['div']} TR={tr}")
        rows.append({**it, **m, "tr": tr})
        time.sleep(1.0)
