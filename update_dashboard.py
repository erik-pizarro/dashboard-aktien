...
def yahoo_symbol(query):
    """Sucht ein Yahoo-Boersensymbol; bevorzugt deutsche Notierungen (.DE etc.)."""
    if not query:
        return None
    ...
    return (de or quotes)[0].get("symbol")
...
def fetch_metrics(wkn, name=None):
    result = {"y1": None, "y5": None, "div": None, "isin": WKN_ISIN.get(wkn)}
    try:
        # Aufloesung in mehreren Stufen: ISIN (Tabelle) -> Name -> WKN
